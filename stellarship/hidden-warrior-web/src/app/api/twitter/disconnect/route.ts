...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendResponse = await fetch(`${BACKEND_URL}/twitter/disconnect`, {
      method: 'POST',
      headers: {
    ...
    if (!backendResponse.ok) {
      return NextResponse.json(
        { message: data.message || 'Failed to disconnect Twitter account' },
    ...
    console.error('Twitter disconnect API error:', error);
    return NextResponse.json(
