...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('🔵 [Twitter Auth] Getting auth URL from backend');
    ...
    const backendUrl = new URL(`${BACKEND_URL}/twitter/auth`);
    if (state) {
      backendUrl.searchParams.set('state', state);
    }

    console.log('🔵 [Twitter Auth] Requesting backend URL:', backendUrl.toString());
    ...
    console.log('🔵 [Twitter Auth] Backend response:', data);

    if (!backendResponse.ok) {
      console.error('🔴 [Twitter Auth] Backend error:', backendResponse.status, data);
      return NextResponse.json(
        { message: data.message || 'Failed to get Twitter auth URL' },
    ...
    if (data.authUrl) {
      ...
      console.log('🔵 [Twitter Auth] Auth URL redirect_uri:', authUrlObj.searchParams.get('redirect_uri'));
    }
