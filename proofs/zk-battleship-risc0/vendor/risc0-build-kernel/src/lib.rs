...
impl KernelBuild {
    ...
    fn compile_cpp(&mut self, output: &str) {
        // NOTE: This crate's `RISC0_SKIP_BUILD_KERNELS` knob is primarily used to
        // skip GPU kernel compilation (Metal/CUDA) when toolchains aren't available.
        // Skipping C++ kernels breaks CPU proving/linking (undefined symbols), so we
        // *always* build C++ kernels even when `RISC0_SKIP_BUILD_KERNELS` is set.
